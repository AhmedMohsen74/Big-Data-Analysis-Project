...
combo= pd.read_csv('bio-decagon-combo.csv')
targets= pd.read_csv('bio-decagon-targets.csv')
gene_ass = pd.read_csv('gene_associations.tsv', delimiter='\t')
disease_ass= pd.read_csv('disease_associations.tsv',delimiter='\t' )
...
def search_Side_Effect_Name_cattegory_First_One(Side_Effect_Name):
  documents= bio_decagon_combo1.find_one({"Side Effect Name": {'$regex': f'.*{Side_Effect_Name}.*', '$options': 'i'}},{"_id":0})["Side Effect Name"]
  disease=decagon_effect1.find({"Side Effect Name":documents},{"_id":0})
  df = pd.DataFrame(list(disease))
  return df

def Side_Effect_Related_Disease_Data(Side_Effect_Name):
    documents= bio_decagon_combo1.find_one({"Polypharmacy Side Effect":{'$regex': f'.*{Side_Effect_Name}.*', '$options': 'i'}},{"_id":0})["Polypharmacy Side Effect"]
    disease=disease_ass1.find({"diseaseId":documents},{"_id":0})
    df = pd.DataFrame(list(disease))
    return df
        ...
        if selected_collection:
            if(selected_collection=='bio_decagon_combo'):
                ...
                if st.button('Search'):
                    ...
                    if st.button('Search Based on Name'):
                            # Search for the text query in MongoDB
                            results = disease_ass_searchName(query2)
                            # Display the results in a DataFrame
                            if len(results) > 0:
                                st.markdown('<p class="description">Search Results:</p>', unsafe_allow_html=True)
                                st.table(results.style.set_table_attributes('class="result-table"'))
